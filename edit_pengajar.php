...
$query = mysqli_query($mysqli, "SELECT * FROM pengajar WHERE teacher_id='$teacher_id'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check if the new teacher_id already exists in the database
    $check_query = mysqli_query($mysqli, "SELECT * FROM pengajar WHERE teacher_id='$new_teacher_id'");
    
    if (mysqli_num_rows($check_query) > 0 && $new_teacher_id != $teacher_id) {
        $error = "Error: teacher_id already exists.";
    } else {
        // Update the record
        $update = mysqli_query($mysqli, "UPDATE pengajar SET full_name = '$full_name', teacher_id = '$new_teacher_id', bidang = '$bidang' WHERE teacher_id = '$teacher_id'");
...
<body>
    ...
    <div class="container">
        ...
        <form action="" method="post">
            ...
            <input type="text" id="teacher_id" name="teacher_id" value="<?php echo htmlspecialchars($data['teacher_id']); ?>" readonly>
            ...
            <select id="bidang" name="bidang" required>
                ...
                <?php while($kursus = $kursus_result->fetch_assoc()) { ?>
                    <option value="<?php echo $kursus['bidang']; ?>"><?php echo $kursus['bidang']; ?></option>
                <?php } ?>
            </select>
